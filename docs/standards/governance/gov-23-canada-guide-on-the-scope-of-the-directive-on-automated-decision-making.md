# Canada — Guide on the scope of the Directive on Automated Decision-Making

## Standard Profile
- **Domain:** Governance
- **Pack ID:** `gov-23-canada-guide-on-the-scope-of-the-directive-on-automated-decision-making`
- **Version:** `1.0`
- **Jurisdiction:** Canada
- **Type:** Guidance

## Official Standard Link
- [Canada — Guide on the scope of the Directive on Automated Decision-Making](https://www.canada.ca/en/government/system/digital-government/digital-government-innovations/responsible-use-ai/guide-scope-directive-automated-decision-making.html)

## Brief Info
Reference checklist pack for Canada — Guide on the scope of the Directive on Automated Decision-Making. Jurisdiction: Canada. Type: Guidance.

## How This Is Used In TrustStack
This standard is represented as a versioned pack and contributes checklist controls, expected evidence, and audit-ready mapping context.
